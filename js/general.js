	...
	if( $("#rewardsA").length){
		...
		$(document).on({click: function () {
		  ...
		  if ($(this).hasClass('checkPrd')) {
			  ...
			  $(ids).find('#totalSpend').html(totalSpend);
			   cnt--;
		  } else {
	...
	if( $("#questionDone").length){
		...
		$(document).on({click: function () {
		  ...
		  if ($(this).hasClass('checkPrd')) {
			   $(this).removeClass('checkPrd');
			   $(this).find('.checked').remove();
			   $('#questionDone .continueBtn').removeClass('enable').addClass("disable");
		  } else {
		 	   $('#questionDone .productBlock .checked').remove();
			   $('#questionDone .productBlock').removeClass('checkPrd');
			   $(this).addClass('checkPrd');
			   $(this).append('<span class="checked">Checked</span>');
			   
			   $('#questionDone .continueBtn').removeClass('disable').addClass("enable");
